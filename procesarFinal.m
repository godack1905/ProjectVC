function mascara_final = procesarFinal(img)
    ...
    %% Circulos en iamgen original si no hay de los otros
    ...
    if sum(yellow_mask(:)) > 300
        ...
        for idx = 1:length(all_yellow_regions)
            ...
            % metodo1 1: Analisis del contorno
            ...
            % Ultima opcion, mirar directamente si es un triangulo
            if ~is_triangle
   ...
   %% ocotogons rojos(stop)
    ...
    if sum(red_mask(:)) > 500
        ...
        % filtrar areas mas grandes solo
        all_regions = {};
        all_areas = [];
        ...
        [~, idx_sorted] = sort(all_areas, 'descend');
        num_to_process = min(10, length(all_regions));
        ...
        for idx = 1:length(all_regions)
            ...
            % Ultimo recurso si no ha detectado aun el octogono
...
%% FUNCIÓN AUXILIAR
% Filtrar regiones muy grande de fondos
